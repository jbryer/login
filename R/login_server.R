...
Wenn Sie nicht angefordert haben, Ihr Passwort zurückzusetzen, können Sie diese E-Mail ignorieren..'
	...
	moduleServer(id, function(input, output, session) {
		...
		get_encrypted_email <- function(email, salt = NULL, salt_algo = "sha512") {
			if(is.null(salt)) {
				return(email)
			} else {
				return(digest::digest(paste0(salt, tolower(email)), algo = salt_algo, serialize = FALSE))
			}
		}
		...
		observeEvent(input$Login, {
			...
			# Suche nach der verschlüsselten Email
